...
namespace Companionship_Barracks
{
    ...
    public class FamilyOnlyRule : ThoughtRule
    {
        ...
        public override (ThoughtDef, Pawn) GetThought(BarracksContext context)
        {
            if (context.NumSiblings == context.NumCohabitants)
                return (CompanionshipBarracksDefOf.SleptInBarracks_OnlySiblings, null);
            if (context.NumParents == context.NumCohabitants)
                return (CompanionshipBarracksDefOf.SleptInBarracks_OnlyParents, null);
            if (context.NumChildren == context.NumCohabitants)
                return (CompanionshipBarracksDefOf.SleptInBarracks_OnlyChildren, null);
    ...
    public class MixedCompanyRule : ThoughtRule
    {
        ...
        public override (ThoughtDef, Pawn) GetThought(BarracksContext context)
        {
            ...
            if (goodFriendRatio >= 0.5f && context.NumEnemies == 0)
                return (CompanionshipBarracksDefOf.SleptInBarracks_GoodFriends_NoEnemies, null);
            if (friendRatio >= 0.5f && context.NumEnemies == 0)
                return (CompanionshipBarracksDefOf.SleptInBarracks_Friends_NoEnemies, null);
            if (goodFriendRatio >= 0.5f && context.NumEnemies > 0)
                return (CompanionshipBarracksDefOf.SleptInBarracks_GoodFriends_WithEnemies, null);
            if (friendRatio >= 0.5f && context.NumEnemies > 0)
                return (CompanionshipBarracksDefOf.SleptInBarracks_Friends_WithEnemies, null);
            if (enemyRatio > 0 && enemyRatio <= 0.33f)
                return (CompanionshipBarracksDefOf.SleptInBarracks_SomeEnemies, null);
